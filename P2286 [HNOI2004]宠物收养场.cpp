...
constexpr int MOD=1000000,inf=0x3f3f3f3f;
std::set<int>s={-inf,inf};
int ans,fl;
signed main(){
//	freopen(".in","r",stdin);
//	freopen(".out","w",stdout);
	for(int T=fio.tpval<int>(),p,x;T--;){
		fio>>p>>x;
		if(p==fl){s.insert(x);continue;}
		if(s.size()==2){s.insert(x);fl=p;continue;}
		auto it=s.upper_bound(x);
		int r=*it,l=*--it;
		if(x-l<=r-x&&l!=-inf)(ans+=x-l)%=MOD,s.erase(l);
