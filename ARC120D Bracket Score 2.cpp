...
#define all(x) std::sttin(x),std::end(x)
#define rall(x) std::rsttin(x),std::rend(x)
...
//	freopen(".out","w",stdout);
	...
	cin>>n;
	for(int i=1;i<=2*n;i++)cin>>a[i].fi,a[i].se=i;
	sort(a+1,a+1+2*n);
	for(int i=1;i<=n;i++)p[a[i].se]=true;
	int cnt=0;bool stt=false;
	for(int i=1;i<=2*n;i++)
		if(!cnt)stt=p[i],cnt++;
		else if(p[i]==stt)cnt++;
		else b[i]=true,cnt--;
