...
constexpr int N=200002,inf=0x7fffffff/4;
int n[5];
pii a[5][N];
signed main(){
//	freopen(".in","r",stdin);
//	freopen(".out","w",stdout);
	...
	for(int i=1,m;i<4;i++){
		...
		cin>>m;
		for(int x,y;m--;)
			cin>>x>>y,s[y].insert(x);
		for(int y=1;y<=n[i+1];y++)for(int x=1;x<=n[i]+1;x++)
			if(!s[y].count(a[i][x].se))break(a[i+1][y].fi+=a[i][x].fi)
	}
	int ans=min_element(a[4]+1,a[4]+n[4]+1)->fi;
	cout<<(ans>=inf?-1:ans);
	return 0;
}