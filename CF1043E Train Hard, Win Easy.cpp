...
//	freopen(".out","w",stdout);
	...
	for(int u,v;m--;)cin>>u>>v,g[u]+=v,g[v]+=u;
	sort(a+1,a+1+n);
	for(int i=1;i<=n;i++)rnk[a[i].id]=i;
	for(int i=1;i<=n;i++)s1[i]=s1[i-1]+a[i].fi;
	for(int i=n;i>=1;i--)s2[i]=s2[i+1]+a[i].se;
	for(int i=1;i<=n;i++){
		ans[a[i].id]+=s1[i-1]+1ll*(i-1)*a[i].se;
		ans[a[i].id]+=s2[i+1]+1ll*(n-i)*a[i].fi;
		for(int j:g[a[i].id]|vw::transform([](int x){return rnk[x];}))
			ans[a[i].id]-=min(a[i].fi+a[j].se,a[i].se+a[j].fi);
	}
