...
inline void add(int u,int v,int w){
	s[find(u)].insert(v);s[find(v)].insert(u);
	g[u][w]+=v;merge(v,g[u][w].front());
	g[v][w]+=u;merge(u,g[v][w].front());
}
inline auto query(int x,int y){
	if(find(x)==find(y))return "Yes\n";
	if(s[find(x)].count(y))return "Yes\n";
	return "No\n";
}
signed main(){
//	freopen(".in","r",stdin);
//	freopen(".out","w",stdout);
	...
	for(int u,v,w;m--;)cin>>u>>v>>w,add(u,v,w);
	for(int u,v;q--;)switch(get<char>()){
	case '+':cin>>u>>v;add(u,v,get<int>());break;
	case '?':cin>>u>>v;cout<<query(u,v);break;
	}
	return 0;
}