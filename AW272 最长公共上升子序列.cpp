...
constexpr int kN=3e3+7;
int a[kN],b[kN],f[kN][kN];
signed main(){
//	freopen(".in","r",stdin);
//	freopen(".out","w",stdout);
	int n,ans=0;fio>>n;
	for(int i=1;i<=n;i++)fio>>a[i];
	for(int i=1;i<=n;i++)fio>>b[i];
	for(int i=1;i<=n;i++)for(int j=1,k=1;j<=n;j++){
		f[i][j]=f[i-1][j];
		if(a[i]==b[j])f[i][j]=std::max(f[i][j],k);
		if(a[i]>b[j])k=std::max(k,f[i-1][j]+1);
	}
