...
int x[M],y[M],t[M],ans=-1;
bool check(int mid){
	for(int i=1;i<=n;i++)
		memset(d[i]+1,0,sizeof(int)*m);
	for(int i=1;i<=q;i++)
		if(t[i]<=mid)add(x[i],y[i],1);
	for(int i=k;i<=n;i++)for(int j=k;j<=m;j++)
		if(ask(i,j)-ask(i-k,j)-ask(i,j-k)+ask(i-k,j-k)==k*k)
			return true;
	return false;
}
signed main(){
...
//	freopen(".out","w",stdout);
	...
	for(int l=0,r=*std::max_element(t+1,t+1+q)+1,mid;l<=r;)
