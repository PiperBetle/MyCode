...
constexpr int kN=14,kM=1<<kN,MOD=1e8;
std::vector<int>s;
int f[kN][kM],a[kN];
signed main(){
//	freopen(".in","r",stdin);
//	freopen(".out","w",stdout);
	int n,m;fio>>n>>m;
	for(int i=1,k;i<=n;i++)for(int j=1;j<=m;j++)fio>>k,a[j]=(a[j]<<1)+!k;
	f[0][0]=1;
	for(int i=0;i<(1<<n);i++)if(!(i&(i<<1)))s.emplace_back(i);
	for(int i=1;i<=m+1;i++){
		for(int j:s)if(!(j&a[i]))for(int k:s){
			if(!(j&k))f[i][j]=(f[i-1][k]+f[i][j])%MOD;
		}
	}
	fio<<f[m+1][0];
	return fio.flush();
