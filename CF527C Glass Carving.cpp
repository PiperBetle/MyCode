...
int w,h,n;
std::multiset<int>dx,dy,lx,ly;
signed main(){
//	freopen(".in","r",stdin);
//	freopen(".out","w",stdout);
	fio>>w>>h>>n;
	lx.insert({0,w});ly.insert({0,h});
	dx.insert(w);dy.insert(h);
	for(int i=1,t;i<=n;i++){
		if(fio.tpval<char>()=='H'){
			fio>>t;auto it=ly.insert(t);
			auto l=*std::prev(it),r=*std::next(it);
			dy.erase(dy.find(r-l));dy.insert({r-t,t-l});
		}else{
			fio>>t;auto it=lx.insert(t);
			auto l=*std::prev(it),r=*std::next(it);
			dx.erase(dx.find(r-l));dx.insert({r-t,t-l});
		}
		fio<<1ll**dx.rbegin()**dy.rbegin()<<'\n';
	}
	return 0;
}