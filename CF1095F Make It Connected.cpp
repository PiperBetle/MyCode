...
using tlii=std::tuple<loli,int,int>;
...
//	freopen(".out","w",stdout);
	...
	int p=int(std::min_element(a+1,a+1+n)-a);
	for(int i=1;i<=n;i++)if(i!=p)e.emplace_back(a[i]+a[p],i,p);
	for(int i=1;i<=m;i++){auto&[w,u,v]=e.emplace_back();cin>>u>>v>>w;}
	sort(all(e));
	for(auto[w,u,v]:e)if((u=find(u))!=(v=find(v)))
		if(fa[u]=v,ans+=w,++cnt==n-1)break;
	cout<<ans;
	return 0;
}