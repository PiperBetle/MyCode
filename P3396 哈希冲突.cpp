...
constexpr int kN=150001,kM=388,siz=10;
int n,m,f[kM][kM],a[kN];
signed main(){
//	freopen(".in","r",stdin);
//	freopen(".out","w",stdout);
	fio>>n>>m;
	for(int i=1;i<=n;i++){fio>>a[i];for(int j=1;j<=siz;j++)f[j][i%j]+=a[i];}
	for(int x,y,sum;m--;)switch(fio.tpval<char>()){
	case 'A':
		fio>>x>>y;
		sum=0;
		if(x<=siz)sum=f[x][y];
	...
	case 'C':
		fio>>x>>y;
		for(int i=1;i<=siz;i++)f[i][x%i]+=y-a[x];
		a[x]=y;
		break;
	}
	return 0;
