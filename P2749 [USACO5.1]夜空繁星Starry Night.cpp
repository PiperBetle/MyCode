...
char f[kN][kN],col='a'-1,c[kN];
using pii=std::pair<int,int>;
std::vector<pii>b[kN];
int n,m,l,a[kN][kN];
double s[kN];
inline double dis(pii x,pii y){return sqrt((x.first-y.first)*(x.first-y.first)+(x.second-y.second)*(x.second-y.second));}
inline int check(int h){
	for(const pii &i:b[l])for(const pii &j:b[l])s[h]+=dis(i,j);
	for(int i=1;i<l;i++)if(fabs(s[i]-s[h])<eps)return i;
	return 0;
}
void dfs(int x,int y){
	for(int dx=-1;dx<=1;dx++)for(int dy=-1;dy<=1;dy++){
		...
		if(!a[nx][ny]||f[nx][ny]!='0')continue;
		f[nx][ny]=col;
		b[l].emplace_back(nx,ny);
		dfs(nx,ny);
	}
}
signed main(){
...
//	freopen("star.out","w",stdout);
	fio>>m>>n;
	for(int i=1;i<=n;i++)for(int j=1;j<=m;j++)a[i][j]=fio.tpval<char>()-'0',f[i][j]='0';
	for(int i=1;i<=n;i++)for(int j=1;j<=m;j++){
		if(f[i][j]!='0'||!a[i][j])continue;
		b[++l].emplace_back(i,j);
		f[i][j]=c[l]=++col;dfs(i,j);
		int flag=check(l);if(!flag)continue;else col--;
		for(const pii &i:b[l])f[i.first][i.second]=c[flag];
	}
	for(int i=1;i<=n;i++,fio<<'\n')for(int j=1;j<=m;j++)fio<<f[i][j];
	return 0;
}