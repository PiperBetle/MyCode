...
std::unordered_map<int,int>fat,val;
inline int find(int x){
	if(!fat.contains(x))return x;
	int fa=find(fat[x]);
	val[x]^=val[fat[x]];
	return fat[x]=fa;
}
...
//	freopen(".out","w",stdout);
	int T=fio.tpval<int>(),x,y,k,last=0;while(T--){
		fio>>k>>x>>y;if(k==1){
			fio>>k;
			x^=last,y^=last,k^=last;
			if(x>y)std::swap(x,y);
			int fx=find(--x),fy=find(y);
			if(fx!=fy)fat[fx]=fy,val[fx]=k^val[x]^val[y];
		}else{
			x^=last,y^=last;
			if(x>y)std::swap(x,y);
			int fx=find(--x),fy=find(y);
			if(fx!=fy)last=1,fio<<"-1\n";
			else fio<<(last=val[x]^val[y])<<'\n';
		}
	}
	return 0;
}