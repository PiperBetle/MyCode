...
//  freopen(".out","w",stdout);
	fio>>n>>m;
	while(n--)s.insert(fio.tpval<int>());
	for(int t,a;m--;){
		fio>>t>>a;switch(t){
		case 1:s.insert(a^lst);break;
		case 2:s.erase(a^lst);break;
		case 3:ans^=(lst=s.order(a^lst));break;
		case 4:ans^=(lst=s[a^lst]);break;
		case 5:ans^=(lst=s.prev(a^lst));break;
		case 6:ans^=(lst=s.next(a^lst));break;
		}
	}
	fio<<ans;
	return 0;
