...
template<typename any>std::istream&get(any x,any y,std::istream&z=cin){for(;x!=y;++x)z>>*x;return z;}
...
//	freopen(".out","w",stdout);
	...
	s=std::accumulate(a+1,a+1+n,0ll);
	dfs1(1,0);
	for(int i=2;i<=n;i++)f[1]+=1ll*dep[i]*a[i];
	dfs2(1,0);
	cout<<*std::max_element(f+1,f+1+n);
	return 0;
}