...
constexpr int N=3e2+1,MOD=1e9+7;
...
//	freopen(".out","w",stdout);
	...
	std::sort(a+1,a+1+n);
	f[0][0][0]=1;
	for(int i=1,same=0;i<=n;i++,same++){
		memset(f[i&1],0,sizeof f[i&1]);
		if(a[i]!=a[i-1]){
			for(int j=0;j<=i;j++)for(int k=1;k<=same;k++)
				add1(f[~i&1][j][0],f[~i&1][j][k]),f[~i&1][j][k]=0;
			same=0;
		}
		for(int j=0;j<=i;j++)for(int k=0;k<=std::min(same,j);k++){
			if(j&&k)add1(f[i&1][j][k],mul0(f[~i&1][j-1][k-1],2*same-k+1));
			add1(f[i&1][j][k],mul0(f[~i&1][j+1][k],j+1-k),mul0(f[~i&1][j][k],i-(2*same-k)-(j-k)));
		}
	}
	cout<<f[n&1][0][0];
	return 0;
