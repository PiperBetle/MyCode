...
constexpr int kN=500007;
std::vector<int>ed[kN];
int sum[kN],f[kN],con[kN];
char c[kN];
std::stack<int>s;
void dfs(int u){
	int t=0;
	if(c[u]==')')if(!s.empty())t=s.top(),con[u]=con[f[t]]+1,s.pop();
	if(c[u]=='(')s.push(u);
	sum[u]=sum[f[u]]+con[u];
...
//	freopen("brackets.out","w",stdout);
	int n,ans=0;f[1]=1;fio>>n>>(c+1);
	for(int i=2,k;i<=n;i++)fio>>k,ed[k].emplace_back(i),f[i]=k;
	dfs(1);for(int i=1;i<=n;i++)ans^=sum[i]*i;
	fio<<ans;
	return fio.flush();
}