...
//	freopen(".out","w",stdout);
	...
	cin>>n>>m;
	for(int i=1,l,r;i<=n;i++)cin>>l>>r,a[l]++,a[r+1]--;
	for(int i=1;i<=m;i++)a[i]+=a[i-1];
	for(int i=1;i<=m;i++){
		if(b1.empty()||a[i]>=b1.back())b1+=a[i];
		else *upper_bound(all(b1),a[i])=a[i];
		f1[i]=siz(b1);
	}
	for(int i=m;i>=1;i--){
		if(b2.empty()||a[i]>=b2.back())b2+=a[i];
		else *upper_bound(all(b2),a[i])=a[i];
		f2[i]=siz(b2);
	}
	for(int i=1;i<=m;i++)
		cmax(ans,f1[i]+f2[i]-1);
	cout<<ans;
	return 0;
}