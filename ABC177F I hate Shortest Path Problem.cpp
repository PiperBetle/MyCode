...
constexpr int kN=2e5+1,inf=0x3f3f3f3f;
...
pii a[kN];
int n,m;
struct odt_node{int l,r;mutable int val;bool operator<(const odt_node &t)const{return l<t.l;}};
struct odt_tree:std::set<odt_node>{
	auto split(int p){
		auto it=lower_bound({p,0,0});
		if(it!=end()&&it->l==p)return it;
		auto t=*--it;erase(it);insert({t.l,p-1,t.val});
		return insert({p,t.r,t.val+p-t.l}).first; 
	}
	...
	auto assign(int l,int r,int k){
		...
		s.erase(it_l,it_r);
		auto it=s.insert({l,r,k}).first;
		if(it==s.begin())return;
		it_l=it,it_r=it;it_l--;it_r++;
		l=it->l,r=it->r,k=it->val;
		if(it_r!=s.end()&&it_l->val==k&&it_r->val==k)l=it_l->l,r=it_r->r,s.erase(it_l,++it_r);
		else if(it_l->val==k)l=it_l->l,s.erase(it_l,++it);
		else if(it_r!=s.end()&&it_r->val==k)r=it_r->r,s.insert(it,++it_r);
		if(l!=r)s.insert({l,r,k});
	}
};
odt_tree s;
signed main(){
...
//	freopen("colorful.out","w",stdout);
	...
	s.insert({0,0,inf});s.insert({m+1,m+1,inf});
	for(int i=1;i<=m;i++)s.insert({i,i,0});
	for(int i=1;i<=n;i++){
		for(auto &it:s)it.val++;
		s.assign(a[i].first,a[i].second,s.split(a[i].first-1)->val+1);
		int ans=inf;for(auto &it:s)chkmin(ans,it.val);
		fio<<(ans<inf?ans:-1)<<'\n';
	}
	return 0;
}