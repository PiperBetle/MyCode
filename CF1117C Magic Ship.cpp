...
bool check(int x){
	...
	p.fi+=y*s[n].fi;
	p.se+=y*s[n].se;
	return abs(p2.fi-p.fi)+abs(p2.se-p.se)<=x;
}
signed main(){
//	freopen(".in","r",stdin);
//	freopen(".out","w",stdout);
	...
	cin>>p1>>p2>>n;
	for(int i=1;i<=n;i++)switch(s[i]=s[i-1],get<char>()){
	case 'U':s[i].se++;break;
	case 'D':s[i].se--;break;
	case 'L':s[i].fi--;break;
	case 'R':s[i].fi++;break;
	}
	int l=1,r=0x3f3f3f3f3f3f3f3f,ans=-1,mid;
	while(l<=r)
		if(check(mid=(l+r)/2))r=mid-1,ans=mid;
		else l=mid+1;
	cout<<ans;
	return 0;
}