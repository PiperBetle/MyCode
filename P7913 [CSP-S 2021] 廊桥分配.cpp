...
int n,m1,m2,ans,c1[N],c2[N];
pii a[N],b[N];
std::set<pii>s;
int calc(){
	for(int res=0,lst=0;;res++){
		auto it=s.upper_bound({lst+1,0});
...
//	freopen(".out","w",stdout);
	...
	s.insert(a+1,a+1+m1);
	for(int i=1;i<=n;i++)c1[i]=c1[i-1]+calc();
	s.clear();
	s.insert(b+1,b+1+m2);
	for(int i=1;i<=n;i++)c2[i]=c2[i-1]+calc();
	for(int i=0;i<=n;i++)cmax(ans,c1[i]+c2[n-i]);
	cout<<ans;
	return 0;
}