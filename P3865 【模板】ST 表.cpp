...
int st[17][100007],lg[100007];
inline int query(int l,int r){int t=lg[r-l+1];return std::max(st[t][l],st[t][r-(1<<t)+1]);}
signed main(){
//	freopen(".in","r",stdin);
//	freopen(".out","w",stdout);
	int n,m,l,r;fio>>n>>m;
	for(int i=1;i<=n;i++)fio>>st[0][i];
	for(int i=2;i<=n;i++)lg[i]=lg[i>>1]+1;
	for(int j=1;(1<<j)<=n;j++)for(int i=1;i+(1<<j)-1<=n;i++)
		st[j][i]=std::max(st[j-1][i],st[j-1][i+(1<<(j-1))]);
	while(m--)fio>>l>>r<<query(l,r)<<'\n';
	return 0;
}