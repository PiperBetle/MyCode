...
int n,k;
loli a[N];
std::unordered_set<loli>f[N][N];
signed main(){
//	freopen(".in","r",stdin);
//	freopen(".out","w",stdout);
	std::ios::sync_with_stdio(false);cin.tie(nullptr);
	cin>>n>>k;
	for(int i=1;i<=n;i++)cin>>a[i],a[i]+=a[i-1],f[i][1].insert(a[i]);
	for(int i=2;i<=k;i++)for(int l=1;l<=n;l++)for(int r=l;r<=n;r++)
		for(loli v:f[l-1][i-1])f[r][i].insert(v&(a[r]-a[l-1]));
	cout<<*std::max_element(all(f[n][k]));
	return 0;
}