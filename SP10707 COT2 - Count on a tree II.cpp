...
constexpr int kN=4e4+1,kM=1e5+1;
int n,m,blk,num,a[kN],b[kN];
int fa[kN],dep[kN],gs[kN],top[kN],sz[kN];
int ans[kM],st[kN<<1],ed[kN<<1],gd[kN<<1],cnt[kN];
bool vis[kN];
...
struct query{int l,r,bl,lca,id;}q[kM];
signed main(){
//	freopen(".in","r",stdin);
//	freopen(".out","w",stdout);
	fio>>n>>m;blk=sqrt(n);
	for(int i=1;i<=n;i++)a[i]=b[i]=fio.tpval<int>();
	std::sort(b+1,b+1+n);
	...
	for(int i=1;i<=m;i++){
		fio>>q[i].l>>q[i].r,q[i].id=i,q[i].lca=LCA(q[i].l,q[i].r);
		if(dep[q[i].l]>dep[q[i].r])std::swap(q[i].l,q[i].r);
		if(q[i].lca==q[i].l)
			q[i].l=st[q[i].l],q[i].r=st[q[i].r],
			q[i].bl=q[i].l/blk,q[i].lca=0;
		else
			q[i].l=ed[q[i].l],q[i].r=st[q[i].r],
			q[i].bl=q[i].l/blk;
	}
	std::sort(q+1,q+1+m,[](const query &x,const query &y){return x.bl==y.bl?(x.bl&1?x.r<y.r:x.r>y.r):x.l<y.l;});
	int sum=0,l=1,r=0;
	auto add=[&](int x){return sum+=(++cnt[a[x]]==1);};
	auto del=[&](int x){return sum-=(--cnt[a[x]]==0);};
	auto calc=[&](int x){vis[x]?del(x):add(x);vis[x]=!vis[x];};
	for(int i=1;i<=m;i++){
		while(l>q[i].l)calc(gd[--l]);
		while(r<q[i].r)calc(gd[++r]);
		while(l<q[i].l)calc(gd[l++]);
		while(r>q[i].r)calc(gd[r--]);
