...
inline int combi(int _x,int _y){return mul0(fac[_x],inv[_y],inv[_x-_y]);}
unsigned val[26];
bsc s;
...
//	freopen(".out","w",stdout);
	...
	for(int i=1;i<=n;i++){
		unsigned k=0;cin>>s;
		for(char c:s)k+=val[c-'a'];
		mp[k]++;
	}
	f[0]=1;
	for(auto[x,y]:mp)for(int i=m;~i;i--)if(f[i])for(int j=y;j;j--)
		if(i+j*(j-1)/2<=m)add1(f[i+j*(j-1)/2],mul0(f[i],combi(y,j)));
	cout<<f[m];
	cout.flush();
	return 0;
}