...
//	freopen("connect.out","w",stdout);
	...
	cin>>n;
	for(int i=1;i<=n;i++)for(int j=f[i][i]=1;j<=n;j++)cin>>a[i][j];
	for(int i=2;i<=n;i++)for(int l=1,r=i;r<=n;l++,r++){
		for(int k=l+1;k<=r;k++)if(a[l][k]){
			int sum=0;
			for(int t=l;t<k;t++)add1(sum,mul0(f[l][t],f[t+1][k]));
			add1(f[l][r],mul0(sum,f[k][r]));
		}
	}
	cout<<f[1][n];
	return 0;
