...
constexpr int N=5e5+1,M=std::__lg(N)+1,inf=0x3f3f3f3f;
int n,m,a[N],b[N],c[N],nxt[M][N];
bsi pre[N];
pii ans;
signed main(){
//	freopen("cmmnjsmnrng.in","r",stdin);
//	freopen("cmmnjsmnrng.out","w",stdout);
	std::ios::sync_with_stdio(false);cin.tie(nullptr);
#define cin fio
#define cout fio
	int T;cin>>T;while(T--){
		cin>>n>>m;ans={inf,inf};
		for(int i=1;i<=n;i++)if(cin>>a[i],a[i]>=N)a[i]=0;
		for(int i=1;i<=m;i++)cin>>b[i];
		for(int i=1;i<m;i++)c[b[i+1]]=b[i];
		for(int i=1;i<=n;i++){
			if(a[i]){
				for(int j:pre[c[a[i]]])nxt[0][j]=i;
				pre[c[a[i]]].clear();
			}
			pre[a[i]]+=i;
		}
		...
		for(int i=1;i<=n;i++)if(a[i]==b[1]){
			...
			for(int i=std::__lg(m);~i;i--)
				if(res>=(1<<i))res-=(1<<i),p=nxt[i][p];
			if(p==0)continue;
			cmin(ans,{p-i+1,i});
		}
		if(ans.fi==inf)cout<<"-1\n";
		else cout<<ans.se<<' '<<ans.se+ans.fi-1<<'\n';
		for(int i=1;i<m;i++)c[b[i+1]]=0;
		for(int i=0;(1<<i)<=n;i++)memset(nxt[i]+1,0,sizeof(int)*n);
		for(int i=1;i<=n;i++)pre[a[i]].clear();
	}
	return 0;
}