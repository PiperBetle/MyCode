...
constexpr int N=1e5+1,M=129,inf=INT_MAX/3;
int n,ans=inf;
loli a[N];
int f[M][M],g[M][M];
signed main(){
...
//	freopen(".out","w",stdout);
	...
	cin>>n;
	for(int i=1;i<=n;i++)if(cin>>a[i],!a[i])i--,n--;
	if(n>=128)return cout<<'3',0;
	for(int i=1;i<=n;i++)for(int j=1;j<=n;j++)
		if(a[i]&a[j]&&i!=j)f[i][j]=g[i][j]=1;
		else f[i][j]=g[i][j]=inf;
	for(int k=1;k<=n;k++){
		for(int i=1;i<=n;i++)for(int j=1;j<=n;j++)
			if(i!=j&&i!=k&&j!=k)cmin(ans,f[i][j]+g[i][k]+g[k][j]);
		for(int i=1;i<=n;i++)for(int j=1;j<=n;j++)
