...
constexpr int kN=2e5+7,kZ=2*std::__lg(kN)*kN;
int n,m,l,a[kN],b[kN],rt[kN],len=0;
struct stn{int ls,rs,lp,rp,data;}s[kZ];
#define nls (s[now].ls)
#define nrs (s[now].rs)
#define mid ((s[now].lp+s[now].rp)>>1)
void build(int &now,int l,int r){
	s[now=++len]={0,0,l,r,0};
	if(l==r)return;
	build(nls,l,mid);build(nrs,mid+1,r);
}
void updata(int &now,int pre,int x){
	s[now=++len]=s[pre];s[now].data++;
	if(s[now].lp==s[now].rp)return;
	x<=mid?updata(nls,s[pre].ls,x):updata(nrs,s[pre].rs,x);
}
int query(int now,int pre,int k){
	if(s[now].lp==s[now].rp)return b[s[now].lp];
	int x=s[s[pre].ls].data-s[nls].data;
	return x>=k?query(nls,s[pre].ls,k):query(nrs,s[pre].rs,k-x);
}
signed main(){
//	freopen(".in","r",stdin);
//	freopen(".out","w",stdout);
	...
	std::sort(b+1,b+1+n);l=std::unique(b+1,b+1+n)-b-1;build(rt[0],1,l);
	for(int i=1;i<=n;i++)updata(rt[i],rt[i-1],std::lower_bound(b+1,b+1+l,a[i])-b);
	for(int l,r,k;m--;)fio>>l>>r>>k<<query(rt[l-1],rt[r],k)<<'\n';
	return 0;
}