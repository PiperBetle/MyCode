...
int n,m=1,mx[N<<2],mn[N<<2],s[N<<2];
#define ls (u<<1)
#define rs (u<<1|1)
#define mid ((l+r)>>1)
void pushup(int u){
	mx[u]=max(mx[ls],mx[rs]+s[ls]);
	mn[u]=min(mn[ls],mn[rs]+s[ls]);
	s[u]=s[ls]+s[rs];
}
void updata(int u,int l,int r,int x,int k){
	if(l==r)return mx[u]=mn[u]=s[u]=k,void();
...
//	freopen(".out","w",stdout);
	...
	for(;n--;){
		switch(get<char>()){
			case 'L':if(m!=1)m--;break;
			case 'R':m++;break;
			case '(':updata(1,1,N-1,m,+1);break;
			case ')':updata(1,1,N-1,m,-1);break;
			default:updata(1,1,N-1,m,0);break;
		}
		if(s[1]||mn[1]<0)cout<<"-1 ";
		else cout<<mx[1]<<' ';
	}
	return 0;
}