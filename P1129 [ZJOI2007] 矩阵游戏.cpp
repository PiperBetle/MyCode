...
constexpr int kN=407;
std::vector<int>g[kN];
int chs[kN],vis[kN],dfs_clock;
bool dfs(int u){
	for(int v:g[u]){
		if(vis[v]==dfs_clock)continue;
		vis[v]=dfs_clock;
...
//	freopen(".out","w",stdout);
	int T;fio>>T;while(T--){
		...
		memset(chs+1,0,sizeof(int)*(n<<1));
		for(int i=1,t;i<=n;i++)for(int j=1;j<=n;j++)
			if(fio>>t,t)g[i].emplace_back(j),g[j].emplace_back(i);
		for(dfs_clock=1;dfs_clock<=n;dfs_clock++)ans+=dfs(dfs_clock);
		fio<<(ans==n?"Yes\n":"No\n");
	}
	return 0;
}