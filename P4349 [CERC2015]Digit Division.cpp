...
struct{
	int d[N];
	void upd(int x,int k){for(x++;x<=n+1;x+=x&-x)add1(d[x],k);}
	int ask(int x){int k=0;for(x++;x;x-=x&-x)add1(k,d[x]);return k;}
	int ask(int x,int y){return sub0(ask(y),ask(x-1));}
}tr;
signed main(){
//	freopen("rnr.in","r",stdin);
//	freopen("rnr.out","w",stdout);
...
#define cout fio
	...
	for(int i=1,g=0;i<=n;i++)if(!(g=(g*10+a[i])%m))p+=i;
	tr.upd(0,1);
	for(int i:p)if(i){
		int p1=lower_bound(all(p),i-r)-p.begin();
		int p2=upper_bound(all(p),i-l)-p.begin()-1;
		if(p1<0||p2<0||p[p1]>p[p2])continue;
		tr.upd(i,tr.ask(p[p1],p[p2]));
	}
	cout<<tr.ask(n,n);
	return 0;
}