...
int n1,n2,m,d,dc,a[N],dfn[N],low[N],bl[N],deg[N],sz[N],ans=1;
bsi g[N];
std::stack<int>s;
void tarjan(int u){
	low[u]=dfn[u]=++dc;
	...
	if(low[u]==dfn[u]){
		bl[u]=++n2;sz[n2]=1;
		for(;s.top()!=u;s.pop())bl[s.top()]=n2,sz[n2]++;
		s.pop();
	}
}
signed main(){
...
//	freopen(".out","w",stdout);
	...
	for(int u,v;m--;){
		cin>>u>>v;
		if((a[u]+1)%d==a[v])g[u]+=v;
		if((a[v]+1)%d==a[u])g[v]+=u;
	}
	for(int i=1;i<=n1;i++)if(!dfn[i])tarjan(i);
	for(int u=1;u<=n1;u++)for(int v:g[u])
		if(bl[u]!=bl[v])deg[bl[u]]++;
	for(int i=1;i<=n2;i++)if(!deg[i])
		if(sz[i]<sz[ans])ans=i;
	cout<<sz[ans]<<'\n';
	for(int i=1;i<=n1;i++)if(bl[i]==ans)cout<<i<<' ';
	return 0;
}