...
int n,m,q,l,f[kN][kM],dep[kN];
using vci=std::vector<int>;
vci ed[kN],ct[kN],g[kN][kM];
inline vci operator+(const vci &x,const vci &y){
	auto i=x.begin(),j=y.begin();vci res;
	while(i!=x.end()&&j!=y.end()&&res.size()<10)res.emplace_back(*i<*j?*i++:*j++);
	while(i!=x.end()&&res.size()<10)res.emplace_back(*i++);
	while(j!=y.end()&&res.size()<10)res.emplace_back(*j++);
	return res;
}
inline void dfs(int u){for(int v:ed[u])if(v!=f[u][0])dep[v]=dep[u]+1,f[v][0]=u,g[v][0]=ct[v],dfs(v);}
inline vci LCA(int x,int y){
	vci res;if(dep[x]>dep[y])std::swap(x,y);
	for(int i=l;i>=0;i--)if(dep[x]+(1<<i)<=dep[y])res=res+g[y][i],y=f[y][i];
	if(x==y)return res+g[y][0];
	for(int i=l;i>=0;i--)if(f[x][i]!=f[y][i])res=res+g[x][i]+g[y][i],x=f[x][i],y=f[y][i];
	if(x!=y)res=res+g[y][1];
	return res+g[x][0];
}
int main(){
	fio>>n>>m>>q;l=std::__lg(n);
	for(int i=1,u,v;i<n;i++)fio>>u>>v,ed[u].emplace_back(v),ed[v].emplace_back(u);
	for(int i=1,t;i<=m;i++){fio>>t;if(ct[t].size()<10)ct[t].emplace_back(i);}
	dep[1]=1;g[1][0]=ct[1];dfs(1);
	for(int j=1;j<=l;j++)for(int i=1;i<=n;i++)f[i][j]=f[f[i][j-1]][j-1],g[i][j]=g[i][j-1]+g[f[i][j-1]][j-1];
	while(q--){
		int u,v,len;fio>>u>>v>>len;
		vci ans=LCA(u,v);fio<<(len=std::min<int>(ans.size(),len));
		for(int i=0;i<len;i++){fio<<' '<<ans[i];}fio<<'\n';
	}
	return 0;
}