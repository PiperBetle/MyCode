...
constexpr int N=101,M=2e6+1;
constexpr loli P=1e9;
int n,m,l[N],r[N],c[N];
loli f[M];
std::deque<int>q[N];
signed main(){
...
//	freopen("road.out","w",stdout);
	...
	for(int i=1;i<=m;i++)for(int j=1;j<=n;j++){
		if(i>=l[j]){
			while(!q[j].empty()&&f[q[j].back()-l[j]]+P*(i-l[j]-q[j].back()+l[j])-c[j]>f[i-l[j]])q[j].pop_back();
			if(!q[j].empty()&&q[j].front()-l[j]<i-r[j])q[j].pop_front();
			q[j].push_back(i);
		}
		if(i>=r[j])
			cmax(f[i],f[q[j].front()-l[j]]+P*(i-q[j].front()+l[j])-c[j]);
	}
	cout<<f[m];
	// 不可以，总司令。
	return 0;
