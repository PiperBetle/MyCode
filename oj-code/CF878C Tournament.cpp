...
struct node{
	int siz,mn[10],mx[10];
	node(int x=1):siz(x){for(int i=0;i<k;i++)mn[i]=mx[i]=fio.tpval<int>();}
	bool operator<(const node &t)const{
		for(int i=0;i<k;i++)if(mx[i]>t.mn[i])return false;
		return true;
	}
	node &operator+=(const node &t){
		siz+=t.siz;
		for(int i=0;i<k;i++)
			mn[i]=std::min(mn[i],t.mn[i]),
			mx[i]=std::max(mx[i],t.mx[i]);
		return *this;
	}
};
std::set<node>s;
