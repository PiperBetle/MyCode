...
int h[kN],a[kN],lmx[kN],rmx[kN],s[kN],top;
signed main(){
//	freopen(".in","r",stdin);
//	freopen(".out","w",stdout);
	...
	while(m--){
		int l,r;fio>>l>>r;r--;
		long long ans=0;s[top=0]=l-1;
		for(int i=l;i<=r;i++){
			while(top&&a[s[top]]<a[i])rmx[s[top]]=i-s[top],top--;
			lmx[i]=i-s[top];s[++top]=i;
		}
		while(top)rmx[s[top]]=r-s[top]+1,top--;
		for(int i=l;i<=r;i++)ans+=1ll*lmx[i]*rmx[i]*a[i];
		fio<<ans<<'\n';
	}
	return fio.flush();
}