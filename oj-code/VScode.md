...
### 指令
这个非常简单，直接使用 cmd 自己输指令编译运行就行了。但是略微有些麻烦，不如 Code Runner。
### Code Runner
...
不过这样弄完之后有些比较麻烦的事情，请看下面 3 个：
#### 不能处理带中括号的文件
这是因为 VScode 默认终端是 PowerShell ，一遇到中括号就萎了。处理方式见下
#### 不能处理带空格的文件
这是因为 Code Runner 默认配置的问题，可以在设置里输入
```json
"terminal.integrated.defaultProfile.windows": "Command Prompt",//将默认终端改成cmd
"code-runner.executorMap": {
	"cpp": "cls && cd $dir && g++ \"$fileName\" -o VScodeRunner -Wl,--stack=21474783647 -O2 -Wall -Wextra -pedantic -std=c++20 && VScodeRunner.exe",
	"ruby": "cls && cd $dir && ruby \"$fileName\"",
	"python": "cls && cd $dir && python -u \"$fileName\""
},
```
按照我这个是 cmd 的语法，所以要把默认终端改成 cmd ，运行后自动清屏，标准是 c++20，栈无限大，编译警告拉满，自动 O2，而且只会产生叫做 VScodeRunner.exe 的文件，避免产生大量无用 .exe 文件影响使用，浪费内存。同时可以处理带有中括号和空格的问题。
#### 不能处理带中文的文件
这个是因为编码问题，要修改就要改编码。  
控制面板-时钟和区域-更改日期、之间或数字格式-管理-更改系统区域设置-使用 Unicode UTF-8 提供全球语言设置前打上勾，确定然后重启电脑就可以解决这个问题。  
但是有一些副作用：一些远古软件的中文会出现乱码，比如 Dev-cpp ，可以把语言改成英文避免此类问题，而且你电脑里有些 .txt 文件会乱码，用 VScode 改编码就行了。另外说一句，如果你之前用 Dev-cpp 写 .cpp 的文件注释，直接用 VScode 打开也是乱码，这个也是编码问题，VScode 默认的编码是 UTF-8，Dev-cpp可能是GB2312。  
...
#### 小优化
众所周知 VScode 在运行时比较慢，使用 Code Runner 运行的速度也是低于 Dev-cpp 的，在台式机上问题不大是秒出，但是在我的垃圾电脑上跑的就比较慢。Code Runner 在编译时期只有编译完才可以输入，有时候着急就按了好几下右键导致粘贴了好几次……这不太好，解决的最好方式是编译结束后输出一条结束语，最简单的方法就是在 settings.json 里插入 echo 语句，不过这样输出是白色的不太好看，所以我选择另一种方式：写一个小程序：
```cpp
#include<iostream>
#include<cstdio>
#include<algorithm>
...
```
VScodeRunner "dkfk  sw3"
```
来检查正确性，如果输出的是蓝色的 `dkfk  sw3` 就表示这个程序没问题。为了 settings.json 的编写方便，我修改了 VScodeRunner.exe 的位置，把这个文件从对应文件夹放到了工作文件夹（也导致 freopen 的相对位置变了，不习惯可以不搞或者两个都搞见机行事），然后把编译完的文件放入工作文件夹改成喜欢的名字比如 IcePrint.exe 然后把 settings.json 这么改：
```cpp
"code-runner.executorMap": {
	// "cpp": "cls && cd $dir && g++ \"$fileName\" -o VScodeRunner -Wl,--stack=2147483647 -O2 -Wall -Wextra -std=c++20 && VScodeRunner",
	"cpp":"cls && g++ $fullFileName -o VScodeRunner -Wl,--stack=21474783647 -O2 -Wall -Wextra -pedantic -std=c++20 && IcePrint \"compile okey, my master ~\" && VScodeRunner.exe"
},
```
结果令人满意QAQ 也可以自己写自己的 IcePrint，甚至可以自己写随机数来控制输出的内容，甚至还能放几句台词？？？反正一切皆有可能只要你会基础 cmd 语法或者基础 c++ 语法。
### launch.json 和 tasks.json
注意：此方法从来没有在 win7 及以下版本成功过，似乎 win7 不能运行 launch.json
#### 内置
这个做法看似麻烦实际不难，而且是考场上没有 Code Runner 时的做法。
首先在刚刚的 .vscode 文件夹里新建 launch.json 文件，选择添加配置-C/C++:(Windows) 启动就会自动生成一串文件，进行修改。
```json
{
	"version": "0.2.0",
	"configurations": [
...
```
非常简短，甚至比生成的还短……然后有的同学又去 F11 或者 F5 了，然后就是报错：找不到任务"g++ compile"。因为我们根本没有配过 tasks.json 。在 .vscode 文件夹里新建文件夹 tasks.json ，输入这个：
```json
{
	"version": "2.0.0",
	"tasks": [
...
```
和上面一样，标准是 c++20，栈无限大，编译警告拉满，自动 O2，而且只会产生叫做 VScodeRunner.exe 的文件。然后直接在要运行的文件上 F5 运行，如有出错，就把终端改成 cmd。这样看似复杂的 launch.json 和 tasks.json 也配置完了。可以在源文件里 F9 设置断点，在调试时可以 Ctrl+Shift+D 监视变量，非常方便。
#### 外置
有了内置就有外置，内置指所有操作都在 VScode 里完成，外置指的是像 Dev-cpp 一样运行时会跳出一个弹窗。既然是仿照 Dev-cpp 就要贺 Dev-cpp 的文件，从 Dev-cpp 的文件夹里复制 ConsolePauser.exe ，用这个文件可以在文件结束后自动暂停，观察文件运行时间和返回值，Dev-cpp 就是通过此文件来暂停的。将该文件放到工作文件夹根目录下（也就是和 .vscode 文件夹同级），然后再 launch.json 里修改一些内容，我就把内置和外置的 launch.json 合并发出来：
```json
{
	"version": "0.2.0",
	"configurations": [
...
```
这样如果在 Ctrl+Shift+D 选择编译并调试就是内置，编译并运行就是外置，一举两得！算上刚刚的 Code Runner，也就是说我们有 3 种方式编译运行 .cpp 文件！  
不过这样还是有个问题：freopen 使用的是相对路径，生成的文件在工作文件夹下，会导致文件混乱，使用 cmd 的指令 pause 代替 ConsolePauser，然后更换工作文件夹即可：
```json
{
	"version": "0.2.0",
	"configurations": [
...
```
不过这样运行文件无法查看返回值和运行时间，真是美中不足QAQ
- 编译并调试(VScode) 内置界面，显示变量名称，不显示返回值和运行时间，freopen 混乱（你都调试了干嘛还 freopen）
- 编译并运行(ConsolePauser) 外置界面，不显示变量名称，显示返回值和运行时间，freopen 混乱（和下面一个正好可以互补）
- 编译并运行(pause) 外置界面，不显示变量名称，不显示返回值和运行时间，freopen 不混乱（和上面一个正好可以互补）
## VSCode git
### git 安装
...
### draw.io
[draw.io](https://draw.io) 是一个免费方便的画图软件，而且支持网页版。Ctrl+Shift+X 输入 drawio 安装自己喜欢的那个插件，只要建一个 xxx.drawio 文件就好了，也是非常方便。
### MindMap
也就是思维导图，只要 Ctrl+Shift+X 输入 vscode-mindmap 安装自己喜欢的那个插件，只要建一个 xxx.km 文件就好了，也是非常方便。
### 用户代码片段
...
[下载链接](https://marketplace.visualstudio.com/items?itemName=zhuangtongfa.Material-theme) 当你装完背景之后 VScode 会贴心提示你 Code 已损坏，安装此插件后 Ctrl+Shift+P 输入 Apply 然后大退就可以去掉 VScode 的贴心提示。
### Colorful Comments
...
我这里把我的settings挂上来，打上注释：
```json
{
	"editor.fontFamily": "Fira Code",//字体为Fira Code
	"editor.insertSpaces": false,//按tab时插入空格
	"editor.fontLigatures": true,//启用连体字
	"debug.onTaskErrors": "showErrors",//运行预启动任务后遇到错误时应该怎么做
	"terminal.integrated.cursorStyle": "line",//控制终端光标的样式
	"terminal.integrated.defaultLocation": "editor",//新建终端的显示位置
	"terminal.integrated.cursorBlinking": true,//终端光标是否闪烁
	"editor.fontSize": 15,//字体大小
	"terminal.integrated.fontSize": 15,//终端字体大小
	"[cpp]": {
		"editor.quickSuggestions": true//代码补全
	},
	"editor.fontWeight": "410",//控制字体粗细
	"terminal.integrated.enableMultiLinePasteWarning": false,//多行粘贴到终端时显示警告
	"window.zoomLevel": -2,//窗口的缩放级别
	"terminal.integrated.defaultProfile.windows": "Command Prompt",//默认终端
	"files.autoGuessEncoding": true,//猜测字符集编码
	"editor.minimap.maxColumn": 100,//缩略图的宽度
	...
	"extensions.ignoreRecommendations": true,//显示扩展建议
	"workbench.activityBar.visible": false,//活动栏的可见性
}
```
这个只是注释不斜体，还有个更生猛的全部禁用斜体：
```cpp
