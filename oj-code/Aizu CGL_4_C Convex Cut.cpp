...
//	freopen(".out","w",stdout);
	...
	while(q--){
		...
		for(size_t i=0;i<a.size();i++){
			if(!(l.p2-l.p1).rightis(a[i]-l.p1))b.emplace_back(a[i]);
			if((l.p2-l.p1).rightis(a[i]-l.p1)!=(l.p2-l.p1).rightis(a[(i+1)%a.size()]-l.p1))
				b.emplace_back(focus({a[i],a[(i+1)%a.size()]},l));
		}
		cout<<b.area()<<'\n';
	}
	return 0;
