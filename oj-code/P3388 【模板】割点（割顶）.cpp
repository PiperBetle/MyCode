...
int n,m,dfn[kN],low[kN],cnt,bl[kN],siz[kN],dfs_clock,ot[kN],ans;
std::vector<int>g[kN];
std::stack<int,std::vector<int>>s;
void chkmin(auto &x,auto y){if(y<x)x=y;}
bool vis[kN];
void tarjan(int u){
...
//	freopen(".out","w",stdout);
	memset(low+1,0x3f,sizeof(int)*n);
	fio>>n>>m;
	for(int i=1,u,v;i<=m;i++)fio>>u>>v,g[u].emplace_back(v);
	for(int i=1;i<=n;i++)if(!dfn[i])dfs_clock=0,tarjan(i);
	for(int i=1;i<=n;i++)for(int v:g[i])if(bl[i]!=bl[v])ot[bl[i]]++;
	for(int i=1;i<=cnt;i++)if(!ot[i]){if(ans!=0)return fio<<0,0;else ans=siz[i];}
	fio<<ans;
	return 0;
}