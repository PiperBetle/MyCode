...
//	freopen(".out","w",stdout);
	int n,a;fio>>n>>a;s.emplace(a,1);
	for(int i=2;i<=n;i++){
		fio>>a;auto it=s.emplace(a,i).first;pii ans(inf,0);
		if(++it!=s.end()&&ans.first>=it->first-a)ans={it->first-a,it->second};
		if((--it)--!=s.begin()&&ans.first>=a-it->first)ans={a-it->first,it->second};
		fio<<ans.first<<' '<<ans.second<<'\n';
	}
	return fio.flush();
}