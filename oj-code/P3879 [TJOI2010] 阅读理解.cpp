...
inline void insert(const std::string &s,const int cnt){
	trie *p=root;
	for(const int it:s|std::views::transform([](const char c){return c-97;})){
		if(!p->s[it])p->s[it]=new trie;
		p=p->s[it];
	}
	if(p->data.empty()||p->data.back()!=cnt)p->data+=cnt;
}
inline void find(const std::string &s){
	trie *p=root;
	for(const int it:s|std::views::transform([](const char c){return c-97;})){
		if(!p->s[it])return;
		p=p->s[it];
	}
	for(int it:p->data)fio<<it<<' ';
}
signed main(){
//	freopen(".in","r",stdin);
//	freopen(".out","w",stdout);
	int n,m;std::string s;
	fio>>n;for(int i=1;i<=n;i++){int t;fio>>t;while(t--)fio>>s,insert(s,i);}
	fio>>m;while(m--){fio>>s;find(s);fio<<'\n';}
	return 0;
}