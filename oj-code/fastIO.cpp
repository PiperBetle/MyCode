...
private:
	...
	bool file_end = false;
	char get() { return p1 == p2 && (p2 = (p1 = ibuf) + fread(ibuf, 1, sizeof ibuf, stdin), p1 == p2) ? (file_end = true), char(EOF) : *p1++; }
	void put(const char x) { p3 - obuf < sizeof obuf ? *p3++ = x : (fwrite(obuf, p3 - obuf, 1, stdout), p3 = obuf, *p3++ = x); }
