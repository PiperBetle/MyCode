...
constexpr int kN=1007;
int a[kN],b[kN],la=1,lb=1,f[kN][kN];
signed main(){
//	freopen(".in","r",stdin);
//	freopen(".out","w",stdout);
	int n,x,y,z;fio>>n>>x>>y>>z;
	for(int i=1,t;i<=n;i++){
		fio>>t;while(t--)a[la++]=i;
		fio>>t;while(t--)b[lb++]=i;
	}
	for(int i=1;i<=la;i++)f[i][0]=i*y;
	for(int i=1;i<=lb;i++)f[0][i]=i*x;
	for(int i=1;i<=la;i++)for(int j=1;j<=lb;j++)
		f[i][j]=std::min({f[i-1][j]+y,f[i][j-1]+x,f[i-1][j-1]+z*abs(a[i]-b[j])});
	fio<<f[la][lb];
	return fio.flush();
}