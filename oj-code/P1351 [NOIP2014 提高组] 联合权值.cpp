...
constexpr int kN=2000007,MOD=10007;
int w[kN],ans1,ans2;
std::vector<int>s[kN];
void dfs(int u,int f,int g){
	int mx1(0),mx2(0),sum(0),squ(0);
	for(int v:s[u]){
		if(v==f)continue;
		sum+=w[v];if(sum>=MOD)sum-=MOD;
		(squ+=w[v]*w[v])%=MOD;
		if(w[v]>mx1)mx2=mx1,mx1=w[v];
		else if(w[v]>mx2)mx2=w[v];
		dfs(v,u,f);
	}
	ans1=std::max({ans1,mx1*mx2,w[u]*w[g]});
	(ans2+=sum*sum-squ+w[u]*w[g]*2)%=MOD;
}
signed main(){
//	freopen(".in","r",stdin);
