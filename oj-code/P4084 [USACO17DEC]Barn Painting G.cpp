...
constexpr int kN=1000007,MOD=1e9+7;
std::vector<int>g[kN];
long long f[kN][3];
inline void dfs(int u,int fa){
	for(int i=0;i<3;i++)if(f[u][i]){for(int j=0;j<i;j++)f[u][j]=0;break;}else f[u][i]=1;
	for(int v:g[u]){
		if(v==fa)continue;
		dfs(v,u);
		(f[u][0]*=(f[v][1]+f[v][2]))%=MOD;
		(f[u][1]*=(f[v][0]+f[v][2]))%=MOD;
		(f[u][2]*=(f[v][0]+f[v][1]))%=MOD;
	}
}
signed main(){
//	freopen(".in","r",stdin);
//	freopen(".out","w",stdout);
	int n,m;fio>>n>>m;
	for(int i=1,u,v;i<n;i++)fio>>u>>v,g[u].emplace_back(v),g[v].emplace_back(u);
	for(int i=1,u,k;i<=m;i++)fio>>u>>k,f[u][k-1]=1;
	dfs(1,0);
	fio<<(f[1][0]+f[1][1]+f[1][2])%MOD;
	return 0;
}