...
constexpr int kN=2e5+7,kZ=2*kN;
int n,m,kl=1;
struct stn{stn *ls,*rs;int data;}*rt[kZ];
#define mid ((l+r)>>1)
void updata(stn *&now,int x,int k,int l,int r){
	if(!now)now=new stn;
	if(l==r){now->data+=k;return;}
	x<=mid?updata(now->ls,x,k,l,mid):updata(now->rs,x,k,mid+1,r);
	now->data=now->ls->data+now->rs->data;
}
int query(stn *now,int x,int y,int l,int r){
	if(l<=x&&y<=r)return now->data;
	if(y<=mid)return query(now->ls,x,y,l,mid);
	if(x>mid)return query(now->rs,x,y,mid+1,r);
	return query(now->ls,x,y,l,mid)+query(now->rs,x,y,mid+1,r);
}
int kth(stn *now,int k,int l,int r){
	if(l==r)return l;
	return now->ls->data>=k?kth(now->ls,k,l,mid):kth(now->rs,k-now->ls->data,mid+1,r);
}
...
void split(stn *&a,stn *&b,int k){
	if(!a)return;
	b=new stn;
	auto v=a->ls->data;
	k>v?split(a->rs,b->rs,k-v):std::swap(a->rs,b->rs);
	if(k<v)split(a->ls,a->ls,k);
	b->data=a->data-k;
	a->data=k;
}
signed main(){
...
//	freopen(".out","w",stdout);
	fio>>n>>m;
	for(int i=1,t;i<=n;i++)if(fio>>t,t)updata(rt[1],i,t,1,n);
	for(int x,y,z,k1,k2;m--;)switch(std::cerr<<"now is"<<m<<'\n',fio.tpval<int>()){
	case 0:
		fio>>x>>y>>z;
		k1=query(rt[x],1,z,1,n),k2=query(rt[x],y,z,1,n);
		rt[0]=nullptr;
		split(rt[x],rt[++kl],k1-k2);
		split(rt[kl],rt[0],k2);
		rt[x]=merge(rt[x],rt[0]);
		break;
	case 1:
		fio>>x>>y;
	...
	case 2:
		...
		break;
	case 3:
		// fio>>x>>y>>z<<query(rt[x],y,z,1,n)<<'\n';
		fio>>x>>y>>z;
		std::cerr<<query(rt[x],y,z,1,n)<<'\n';
		break;
	case 4:
		fio>>x>>y<<(rt[x]->data<y?-1:kth(rt[x],y,1,n));
		break;
	}
	return 0;
}