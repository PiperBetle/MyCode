...
constexpr int kN=2e5+1,inf=0x2a2a2a2a;
int n,m,A,B,C,dis[3][kN];
long long w[kN],ans;
std::basic_string<int>g[kN];
void bfs(int stt,int (&dis)[kN]){
	memset(dis+1,0x2a,sizeof(int)*n);dis[stt]=0;
	for(std::queue<int>q({stt});!q.empty();){
		int u=q.front();q.pop();
		for(int v:g[u])if(dis[v]==inf)dis[v]=dis[u]+1,q.emplace(v);
	}
}
signed main(){
//	freopen(".in","r",stdin);
//	freopen(".out","w",stdout);
	for(int T=fio.tpval<int>();T--;){
		fio>>n>>m>>A>>B>>C;ans=LONG_LONG_MAX;
		for(int i=1;i<=m;i++)fio>>w[i];
		std::sort(w+1,w+1+m);
		for(int i=1;i<=m;i++)w[i]+=w[i-1];
		for(int i=1;i<=n;i++)g[i].clear();
		for(int i=1,u,v;i<=m;i++)fio>>u>>v,g[u]+=v,g[v]+=u;
		bfs(A,dis[0]);bfs(B,dis[1]);bfs(C,dis[2]);
		for(int i=1;i<=n;i++)if(int t=dis[0][i]+dis[1][i]+dis[2][i];t<=m)
			ans=std::min(ans,w[t]+w[dis[1][i]]);
		fio<<ans<<'\n';
	}
	return 0;
}