...
constexpr int kN=107,kM=1007;
std::basic_string<int>g[kM];
int pl[kN],cnt[kM];
std::stack<int>s;
bool vis[kM];
signed main(){
...
//	freopen(".out","w",stdout);
	int k,n,m;fio>>k>>n>>m;
	for(int i=1;i<=k;i++)fio>>pl[i];
	for(int i=1,u,v;i<=m;i++)fio>>u>>v,g[u]+=v;
	for(int i=1;i<=k;i++){
		memset(vis+1,0,sizeof(bool)*n);
		for(s.emplace(pl[i]),vis[pl[i]]=true,cnt[pl[i]]++;!s.empty();){
			int u=s.top();s.pop();
			for(int v:g[u])if(!vis[v])s.emplace(v),vis[v]=true,cnt[v]++;
		}
	}
	fio<<std::count(cnt+1,cnt+1+n,k);
	return 0;
}