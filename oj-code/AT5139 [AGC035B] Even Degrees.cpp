...
std::basic_string<int>g2[kN];
std::vector<pii>b,g1[kN];
bool vis[kN],use[kN];
int n,m,len,ot[kN],u[kN],v[kN];
void dfs(int u){
	vis[u]=true;
	for(auto[v,id]:g1[u]){
		if(vis[v])continue;
...
//	freopen(".out","w",stdout);
	fio>>n>>m;
	if(m&1)return fio<<"-1",0;
	for(int i=1;i<=m;i++)fio>>u[i]>>v[i],g1[u[i]].emplace_back(v[i],i),g1[v[i]].emplace_back(u[i],i);
	dfs(1);
	for(int i=1;i<=m;i++)if(!use[i])ot[u[i]]++,fio<<u[i]<<' '<<v[i]<<'\n';
	solve(1,0);
	return 0;
}