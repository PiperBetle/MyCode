...
struct vector{
	...
	vector rotate(const vector &a,const double &r){
		auto u=a-*this;
		return *this+vector{u.x*cos(r)-u.y*sin(r),u.x*sin(r)+u.y*cos(r)};
	}
};
vector polar(const double &p,const double &r){return {cos(r)*p,sin(r)*p};}
struct segment{
	vector p1,p2;
...
struct circle{
	...
	segment tangent(const vector &a){
		auto u=a-p;
		auto l=acos(r/~u);
		u=u*r/~u;
