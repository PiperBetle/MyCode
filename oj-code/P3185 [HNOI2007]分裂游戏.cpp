...
//	freopen(".out","w",stdout);
	int T=fio.tpval<int>();while(T--){
		int n=fio.tpval<int>(),ans=0,cnt=0;
		sg[n]=0;for(int i=1;i<=n;i++)fio>>a[i];
		for(int i=n-1;i>=1;i--){
			memset(vis,0,sizeof vis);
			for(int j=i+1;j<=n;j++)for(int k=j;k<=n;k++)vis[sg[j]^sg[k]]=true;
			for(int j=0;;j++)if(!vis[j]){sg[i]=j;break;}
		}
		for(int i=1;i<=n;i++)if(a[i]&1)ans^=sg[i];
		for(int i=1;i<=n;i++)if(a[i])for(int j=i+1;j<=n;j++)for(int k=j;k<=n;k++)
			if((sg[i]^sg[j]^sg[k])==ans&&!cnt++)fio<<i-1<<' '<<j-1<<' '<<k-1<<'\n';
		if(!cnt)fio<<"-1 -1 -1\n";
		fio<<cnt<<'\n';
	}
	return 0;
