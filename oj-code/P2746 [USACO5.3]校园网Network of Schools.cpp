...
constexpr int kN=1e2+7;
int n,dfs_clock,low[kN],dfn[kN],cnt,siz[kN],bl[kN],in[kN],ot[kN];
bool vis[kN];
std::vector<int>g[kN];
std::stack<int>s;
...
//	freopen(".out","w",stdout);
	fio>>n;
	for(int u=1,v;u<=n;u++)while(fio>>v,v)g[u].emplace_back(v);
	for(int i=1;i<=n;i++)if(!dfn[i])tarjan(i);
	for(int u=1;u<=n;u++)for(int v:g[u])if(bl[u]!=bl[v])ot[bl[u]]++,in[bl[v]]++;
	for(int i=1;i<=cnt;i++)ot[0]+=!ot[i],in[0]+=!in[i];
	fio<<in[0]<<'\n'<<(cnt==1?0:std::max(in[0],ot[0]));
	return 0;
}