...
int main()
{
	...
	int ssro=0,sro=0,ro=90,so=90,bo=0,co=0,gano=0,zheno=0,weio=0,shengo=0,foodo=0,watero=0;//数量 
	...
	for(i=1;i<=tianshu;i++)
	{
											...
											else
											{
												...
												else
												{
													...
													else
													{
														if(t=='b')
														{
															cout<<"是否来一局三子棋,你赢你+100,否则-100 1.是 2.不是"<<endl;
															cin>>t;
															if(t=='1')
															{
																for(i=1;i<=3;i++)
		...
		if(jing<=333)
		{
			if(rand()%8==0)
			{
				...
				cout<<"体力:"<<chi<<" 水分:"<<he<<" 精神:"<<jing<<" 属于";
				if(jing>=667)cout<<"正常"<<endl;else if(jing<=333)cout<<"疯狂"<<endl;else cout<<"抑郁"<<endl;
				cout<<"天数:"<<i<<" 时间:"<<(end-begin)/60000<<"分"<<((end-begin)/1000)%60<<"秒"<<endl;
				cout<<"你";
				if(flag==1)cout<<"十分健康"<<endl;else cout<<"得了"<<as[zzxh]<<endl;
				system("pause");
				exit(1);
	...
	cout<<"体力:"<<chi<<" 水分:"<<he<<" 精神:"<<jing<<" 属于";
	if(jing>=667)cout<<"正常"<<endl;else if(jing<=333)cout<<"疯狂"<<endl;else cout<<"抑郁"<<endl;
	cout<<"天数:"<<tianshu<<" 时间:"<<(end-begin)/60000<<"分"<<((end-begin)/1000)%60<<"秒"<<endl<<"你";
	if(flag==1)cout<<"十分健康"<<endl;else cout<<"得了"<<as[zzxh]<<endl;
	system("pause");
