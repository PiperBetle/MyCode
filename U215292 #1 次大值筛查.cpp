...
{
private:
	char ibuf[1007], *p1 = ibuf, *p2 = ibuf, obuf[1007], *p3 = obuf, sta[50];
	bool file_end = false;
	char get() { return p1 == p2 && (p2 = (p1 = ibuf) + fread(ibuf, 1, 1007, stdin), p1 == p2) ? (file_end = true), EOF : *p1++; }
	void put(char x) { p3 - obuf < 1007 ? *p3++ = x : (fwrite(obuf, p3 - obuf, 1, stdout), p3 = obuf, *p3++ = x); }
...
public:
	...
	{
		t.clear();
		for (char c = get(); !isspace(c); c = get())
			t.push_back(c);
		return *this;
	}
