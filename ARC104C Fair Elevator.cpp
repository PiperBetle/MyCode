...
//	freopen(".out","w",stdout);
	...
	for(int i=1;i<=n;i++){
		cin>>a[i]>>b[i];
		if(a[i]!=-1&&b[i]!=-1&&a[i]>=b[i])return cout<<"No",0;
		if(a[i]!=-1&&(p1[a[i]]||p2[a[i]]))return cout<<"No",0;
		if(b[i]!=-1&&(p1[b[i]]||p2[b[i]]))return cout<<"No",0;
		if(a[i]!=-1)p1[a[i]]=true;
		if(b[i]!=-1)p2[b[i]]=true;
		if(a[i]!=-1)l[a[i]]=b[i];
		if(b[i]!=-1)r[b[i]]=a[i];
	}
	for(int i=1;i<=2*n;i++){
		for(int j=i&1;j<i;j+=2)if(f[j]){
			int d=(i-j)/2;
			for(int k=j+1;k<=j+d;k++)
				if((l[k]&&l[k]!=-1&&l[k]!=k+d)||p2[k]||(p1[k]&&l[k]==-1&&p2[k+d]&&r[k+d]==-1))
					goto impart;
			for(int k=j+d+1;k<=i;k++)
				if((r[k]&&r[k]!=-1&&r[k]!=k-d)||p1[k]||(p1[k-d]&&l[k-d]==-1&&p2[k]&&r[k]==-1))
					goto impart;
			f[i]=true;
			impart:;
		}
