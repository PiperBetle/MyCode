...
struct memory{int vs,id;auto operator<=>(const memory &)const=default;};
namespace pbds=__gnu_pbds;
pbds::priority_queue<memory,std::greater<>>q;
int n,m,l,a[kN],b[kN],cnt[kN],ans,tot;
signed main(){
//	freopen(".in","r",stdin);
//	freopen(".out","w",stdout);
	fio>>n>>m;
	for(int i=1;i<=m;i++)a[i]=b[i]=fio.tpval<int>();
	std::sort(b+1,b+1+m);
	l=std::unique(b+1,b+1+m)-b-1;
	for(int i=1;i<=m;q.push({++cnt[a[i]],i}),i++){
		a[i]=std::lower_bound(b+1,b+1+l,a[i])-b;
		if(cnt[a[i]]){ans++;continue;}
		if(tot<n){tot++;continue;}
		auto t=q.top();q.pop();
		while(cnt[a[t.id]]!=t.vs)t=q.top(),q.pop();
		cnt[a[t.id]]=0;
	}
	fio<<ans;
	return 0;
