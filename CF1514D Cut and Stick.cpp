...
constexpr int kN=3e5+7,kZ=1.5*std::__lg(kN)*kN;
int n,m,ed,rt[kN],len=0;
...
#define prs (s[pre].rs)
#define mid ((l+r)>>1)
void updata(int &now,int pre,int x,int l,int r){
	s[now=++len]=s[pre];s[now].data++;
	if(l==r)return;
	x<=mid?updata(nls,pls,x,l,mid):updata(nrs,prs,x,mid+1,r);
}
int query(int now,int pre,int l,int r){
	if(l==r)return s[pre].data-s[now].data;
	return s[pls].data-s[nls].data>=s[prs].data-s[nrs].data?query(nls,pls,l,mid):query(nrs,prs,mid+1,r);
}
signed main(){
	fio>>n>>m;
	for(int i=1;i<=n;i++)updata(rt[i],rt[i-1],fio.tpval<int>(),1,n);
	for(int l,r;m--;)fio>>l>>r<<std::max(1,query(rt[l-1],rt[r],1,n)*2-(r-l+1))<<'\n';
	return 0;
}