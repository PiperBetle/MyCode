...
constexpr int P=998244353,N=4e5+1,M=301;
struct mint{
	int d;
	mint()=default;
...
int n,k;
mint a[N],fac[M],ifc[M],sum[M],pw[M];
signed main(){
//	freopen(".in","r",stdin);
//	freopen(".out","w",stdout);
	...
	ifc[M-1]=fac[M-1].inv();
	for(int i=M-2;i>=2;i--)ifc[i]=ifc[i+1]*(i+1);
	sum[0]=n;
	for(int i=1;i<=n;i++){
		mint v1=1,v2=1;
		for(int j=1;j<=k;j++){
	...
	for(int j=1;j<=k;j++){
		mint ans=0;
		for(int i=0;i<=j;i++)ans+=sum[i]*sum[j-i];
		cout<<(ans*fac[j]-pw[j])/2<<'\n';
	}
	return 0;
}