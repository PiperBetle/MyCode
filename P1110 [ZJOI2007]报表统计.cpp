...
constexpr int kN=5e5+1,inf=0x3f3f3f3f;
int n,m,ans=inf;
using pii=std::pair<int,int>;
pii a[kN];
std::multiset<int>delta,full({-inf,inf});
void updata(int x){
	auto it=full.lower_bound(x);
	auto r=*it,l=*--it;
	ans=std::min({ans,r-x,x-l});
	full.insert(x);
}
void change(int p,int x){
	delta.insert(abs(a[p].second-x));
	if(p!=n)delta.insert(abs(a[p+1].first-x)),delta.erase(delta.find(abs(a[p+1].first-a[p].second)));
	a[p].second=x;
}
signed main(){
//	freopen(".in","r",stdin);
//	freopen(".out","w",stdout);
	fio>>n>>m;
	for(int i=1;i<=n;i++)a[i].first=a[i].second=fio.tpval<int>();
	for(int i=1;i<n;i++)delta.insert(abs(a[i+1].first-a[i].second));
	for(int i=1;i<=n;i++)updata(a[i].first);
	for(int p,x;m--;)switch(fio.tpval<std::string>().size()){
	case 6:fio>>p>>x;updata(x);change(p,x);break;
	case 7:fio<<*delta.begin()<<'\n';break;
	case 12:fio<<ans<<'\n';break;
	}
	return 0;
}