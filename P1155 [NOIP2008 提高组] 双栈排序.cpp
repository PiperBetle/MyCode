...
constexpr int kN=1e3+7;
int n,a[kN],c[kN],m[kN],pos=1;
std::vector<int>g[kN];
std::stack<int>s[2];
inline bool pop(int id){
	if(!s[id].empty()&&s[id].top()==pos){
		fio<<(id?'d':'b')<<' ';s[id].pop();pos++;
		return 1;
	}
	return 0;
}
inline void push(int p,int id){
	if(id==1)while(pop(0));
	while(!s[id].empty()&&s[id].top()<p)if(!pop(id))pop(id^1);
	if(id==1)while(pop(0));
	s[id].push(p);fio<<(id?'c':'a')<<' ';
}
signed main(){
//	freopen(".in","r",stdin);
//	freopen(".out","w",stdout);
	...
	m[n+1]=n+1;
	for(int i=n;i>=1;i--)m[i]=std::min(m[i+1],a[i]);
	for(int i=1;i<=n;i++)for(int j=i+1;j<=n;j++)
		if(m[j+1]<a[i]&&a[i]<a[j])g[i].emplace_back(j),g[j].emplace_back(i),c[i]=c[j]=-1;
	for(int i=1;i<=n;i++)if(!~c[i]){
		std::queue<int>q({i});c[i]=0;
		while(!q.empty()){
			int u=q.front();q.pop();
			for(int v:g[u]){
				if(~c[v]&&c[v]!=(c[u]^1))return fio<<0,0;
				if(!~c[v])q.emplace(v);
				c[v]=c[u]^1;
			}
		}
	}
	for(int i=1;i<=n;i++)push(a[i],c[i]);
	for(int i=1;i;){
		i=0;
		while(pop(0))i=1;
		while(pop(1))i=1;
