...
constexpr int kN=5007;
int n,m,d,dc,l,p[kN],c[kN],use[kN],k[kN],mc[kN],ans[kN],vis[kN];
std::basic_string<int>g[kN];
bool dfs(int u){
	for(int v:g[u]){
		if(vis[v]==dc)continue;
		vis[v]=dc;
		if(mc[v]==-1||dfs(mc[v]))return mc[v]=u,true;
	}
	return false;
}
signed main(){
...
//	freopen(".out","w",stdout);
	fio>>n>>m;
	for(int i=1;i<=n;i++)fio>>p[i];
	for(int i=1;i<=n;i++)fio>>c[i];
	fio>>d;
	for(int i=1;i<=d;i++)use[k[i]=fio.tpval<int>()]=true;
	for(int i=1;i<=n;i++)if(!use[i])g[p[i]]+=c[i];
