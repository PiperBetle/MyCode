...
constexpr int N=2e5+1,P=998244353;
struct mint{
	int d;
	mint()=default;
	mint(int x):d(x<0?x+P:x){}
	mint(long long x):d(int(x%P)){}
	...
	static mint qpow(int x,long long y=P-2){int z=1;for(;y;y>>=1,x=int(1ll*x*x%P))if(y&1)z=int(1ll*x*z%P);return z;}
