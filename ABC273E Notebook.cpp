...
cxx::rope<int>a;
std::map<int,cxx::rope<int>>f;
signed main(){
//	freopen(".in","r",stdin);
//	freopen(".out","w",stdout);
	std::ios::sync_with_stdio(false);cin.tie(nullptr);
	int T;cin>>T;for(;T--;cout<<(a.empty()?-1:a.back())<<' ')
	switch(get<bsc>().front()){
		case 'A':a+=get<int>();break;
		case 'D':if(!a.empty())a.pop_back();break;
		case 'S':f[get<int>()]=a;break;
		case 'L':a=f[get<int>()];break;
	}
	return 0;
}