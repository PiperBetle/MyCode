...
constexpr int kN=200007,kZ=kN*std::__lg(kN)*4;
int n,m,len,ans[kN],rt[101];
struct stn{int ls=0,rs=0;}s[kZ];
#define mid ((l+r)>>1)
int merge(int a,int b,int l,int r){
	if(!a||!b)return a|b;
	if(l==r)return a;
	s[a].ls=merge(s[a].ls,s[b].ls,l,mid);
	s[a].rs=merge(s[a].rs,s[b].rs,mid+1,r);
	return a;
}
void updata(int &now,int l,int r,int x){
	if(!now)now=++len;
	if(l==r)return;
	x<=mid?updata(s[now].ls,l,mid,x):updata(s[now].rs,mid+1,r,x);
}
void change(int &a,int &b,int l,int r,int x,int y){
	if(!a)return;
	if(x<=l&&r<=y){b=merge(a,b,l,r);a=0;return;}
	if(!b)b=++len;
	if(x<=mid)change(s[a].ls,s[b].ls,l,mid,x,y);
	if(y>mid)change(s[a].rs,s[b].rs,mid+1,r,x,y);
}
void print(int now,int l,int r,int x){
	if(!now)return;
	if(l==r){ans[l]=x;return;}
	print(s[now].ls,l,mid,x);
	print(s[now].rs,mid+1,r,x);
}
signed main(){
//	freopen(".in","r",stdin);
//	freopen(".out","w",stdout);
	fio>>n;
	for(int i=1;i<=n;i++)updata(rt[fio.tpval<int>()],1,n,i);
	fio>>m;
	for(int l,r,x,y;m--;){fio>>l>>r>>x>>y;if(x!=y)change(rt[x],rt[y],1,n,l,r);}
	for(int i=1;i<=100;i++)print(rt[i],1,n,i);
