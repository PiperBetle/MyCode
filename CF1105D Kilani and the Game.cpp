...
constexpr int N=1002,dx[]={-1,1,0,0},dy[]={0,0,-1,1};
std::queue<tiii>q[10];
int n,m,p,a[N],b[N];
char c[N][N];
signed main(){
...
//	freopen(".out","w",stdout);
	...
	for(int i=1;i<=n;i++)for(int j=1;j<=m;j++)
		if(isdigit(c[i][j]=get<char>()))
			q[c[i][j]-'0'].emplace(i,j,0),
			b[c[i][j]-'0']++;
	for(;;){
		bool brk=true;
		for(int i=1;i<=p;i++)if(!q[i].empty())brk=false;
		if(brk)break;
		for(int i=1;i<=p;i++)for(int T=a[i];T--;){
			if(q[i].empty())break;
			int d=get<2>(q[i].front());
			while(!q[i].empty()){
				auto[x,y,l]=q[i].front();
				if(l!=d)break;else q[i].pop();
				for(int j=0;j<4;j++){
					int nx=x+dx[j],ny=y+dy[j];
					if(c[nx][ny]=='.'){
						c[nx][ny]=char('0'+i);
						b[i]++;
						q[i].emplace(nx,ny,d+1);
					}
				}
			}
		}
