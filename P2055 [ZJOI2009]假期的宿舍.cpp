...
constexpr int kN=51;
int n,dc,sc[kN],hm[kN],g[kN][kN],chs[kN],vis[kN];
bool dfs(int u){
	for(int v=1;v<=n;v++){
		if(vis[v]==dc||!sc[v]||!g[u][v])continue;
		vis[v]=dc;
...
//	freopen(".out","w",stdout);
	int T;fio>>T;while(T--){
		fio>>n;
		memset(sc+1,0,sizeof(int)*n);
		memset(hm+1,0,sizeof(int)*n);
		memset(chs+1,0,sizeof(int)*n);
		memset(vis+1,0,sizeof(int)*n);
		for(int i=1;i<=n;i++)fio>>sc[i],memset(g[i]+1,0,sizeof(int)*n);
		for(int i=1;i<=n;i++)if(fio>>hm[i],!sc[i])hm[i]=1;
		for(int i=1;i<=n;i++){for(int j=1;j<=n;j++)fio>>g[i][j];if(sc[i])g[i][i]=1;}
		fio<<[]{for(dc=1;dc<=n;dc++)if((!sc[dc]||!hm[dc])&&!dfs(dc))return "T_T\n";return "^_^\n";}();
	}
	return 0;
}