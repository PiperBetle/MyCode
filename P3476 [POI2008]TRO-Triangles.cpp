...
struct vector{
	...
	vector(const int &a=0,const int &b=0):x(a),y(b){}
	int operator~(){return sqrt(x*x+y*y);}
	...
	friend vector operator-(const vector &a,const vector &b){return {a.x-b.x,a.y-b.y};}
	vector &operator-=(const vector &t){x-=t.x,y-=t.y;return *this;}
	friend vector operator*(const vector &a,const int &b){return {a.x*b,a.y*b};}
	vector &operator*=(const int &t){x*=t,y*=t;return *this;}
	friend vector operator/(const vector &a,const int &b){return {a.x/b,a.y/b};}
	vector &operator/=(const int &t){x/=t,y/=t;return *this;}
	friend int operator*(const vector &a,const vector &b){return a.x*b.x+a.y*b.y;}
	friend int operator/(const vector &a,const vector &b){return a.x*b.y-a.y*b.x;}
};
constexpr int kN=3e3+1;
int n;
vector a[kN];
signed main(){
//	freopen("tkk.in","r",stdin);
//	freopen("tkk.out","w",stdout);
	fio>>n;
	...
	for(int i=1;i<=n;i++){
		std::vector<vector>b;
		for(int j=i+1;j<=n;j++)b.emplace_back(a[j]-a[i]);
		std::sort(b.begin(),b.end(),[](cauto &x,cauto &y){return x/y<0;});
		vector sum=0;
		for(int j=0;j<siz(b);j++)ans+=sum/b[j],sum-=b[j];
	}
	fio<<ans/2<<(ans&1?".5":".0");
	return 0;
}