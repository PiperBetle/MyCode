...
//	freopen(".out","w",stdout);
	...
	cin>>n>>q;for(int i=1;i<=n;i++)cin>>a[i];s.insert(a+1,a+1+n);
	for(int x;q--;)switch(get<int>()){
	case 1:cin>>x;if(s.contains(x))s.erase(s.find(x)),cout<<2*(*s.rbegin()-*s.begin())<<'\n';else cout<<"-1\n";break;
	case 2:cin>>x;s.insert(x);cout<<2*(*s.rbegin()-*s.begin())<<'\n';break;
	}
	return 0;
}