...
int n,P,pw[15],a[N],lg[N],ans;
std::unordered_map<int,int>mp[15];
signed main(){
//	freopen(".in","r",stdin);
//	freopen(".out","w",stdout);
	std::ios::sync_with_stdio(false);cin.tie(nullptr);
	cin>>n>>P;
	for(int i=pw[0]=1;i<=10;i++)pw[i]=10*pw[i-1]%P;
	for(int i=1;i<=n;i++){
		cin>>a[i];
		for(int k=a[i];k;k/=10)lg[i]++;
	}
