...
constexpr int N=31,M=101,MOD=998244353;
int n,m,k,f[M][N][N][N],c[N][N],v[M][N];
...
int dfs(int x,int y,int a,int b){
	if(y>=n)return a+std::__popcount(b)<=k;
	if(x>m)return 0;
	if(~f[x][y][a][b])return f[x][y][a][b];
	for(int i=0;i<=n-y;i++)
		res=(res+dfs(x+1,y+i,a+(b+i&1),b+i>>1)*p[x][i]*c[n-y][i]);
}
signed main(){
//	freopen(".in","r",stdin);
//	freopen(".out","w",stdout);
	...
	memset(f,-1,sizeof f);
	cin>>n>>m>>k;
	c[0][0]=1;for(int i=1;i<=n;i++)for(int j=c[i][0]=1;j<=i;j++)c[i][j]=add0(c[i-1][j-1],c[i-1][j]);
	for(int i=1;i<=m;i++){v[i][0]=1;cin>>v[i][1];for(int j=2;j<=n;j++)v[i][j]=mul0(v[i][j-1],v[i][1]);}
	return 0;
}