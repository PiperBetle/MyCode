...
constexpr int K=16,N=201,inf=0x3f3f3f3f;
int n,k,f[1<<K|1];
...
//	freopen(".out","w",stdout);
	...
	cin>>n>>k;
	memset(f+1,0x3f,sizeof(int)*((1<<k)-1));
	for(int i=1,x;i<=n;i++){
		cin>>x;x--;
		for(int j=(1<<k)-1;~j;j--)if(f[j]!=inf){
			if(~j>>x&1)cmin(f[j|(1<<x)],f[j]+calc(j&(~((1<<x)-1))));
			f[j]+=min(calc(j),k-calc(j));
		}
	}
	cout<<f[(1<<k)-1];
	return 0;
