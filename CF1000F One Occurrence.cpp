...
int n,m,blk,a[kN],ans[kN],cid[kN],cnt[kN];
struct query{int bl,l,r,id;}q[kN];
std::vector<int>s;
signed main(){
//	freopen(".in","r",stdin);
//	freopen(".out","w",stdout);
	fio>>n;blk=sqrt(n);s.reserve(n);
	for(int i=1;i<=n;i++)fio>>a[i];
	fio>>m;
	for(int i=1;i<=m;i++)fio>>q[i].l>>q[i].r,q[i].id=i,q[i].bl=(q[i].l-1)/blk+1;
	std::sort(q+1,q+1+m,[](const query &x,const query &y){return x.bl==y.bl?(x.bl&1?x.r<y.r:x.r>y.r):x.l<y.l;});
	auto add=[](int x){
		if(cnt[a[x]]==1)s[cid[a[x]]]=s.back(),cid[s.back()]=cid[a[x]],s.pop_back();
		if(++cnt[a[x]]==1)s.emplace_back(a[x]),cid[a[x]]=s.size()-1;
	};
	auto del=[](int x){
		if(cnt[a[x]]==1)s[cid[a[x]]]=s.back(),cid[s.back()]=cid[a[x]],s.pop_back();
		if(--cnt[a[x]]==1)s.emplace_back(a[x]),cid[a[x]]=s.size()-1;
